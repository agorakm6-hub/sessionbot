...
async def load_sessions_on_startup() -> None:
    ...
    if not raw:
        logger.info("ℹ️ Сохранённых сессий для жалоб не найдено")
        return
    loaded = 0
    for name, session_string in raw.items():
        try:
    ...
    logger.info(f"✅ Загружено сессий для жалоб: {loaded}/{len(raw)}")
...
async def send_message_report(client, peer, message_id, reason_object, comment_text):
    try:
        ...
        await asyncio.wait_for(
            client(ReportRequest(peer=input_peer, id=[message_id], option=b'', message=comment_text)),
            timeout=20
        )
        return True
...
async def send_peer_report(client, peer, reason_object, comment_text):
    try:
        ...
        input_peer = await client.get_input_entity(entity)
        await asyncio.wait_for(
            client(ReportPeerRequest(peer=input_peer, reason=reason_object, message=comment_text)),
            timeout=20
        )
        return True
    except Exception as error:
