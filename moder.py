...
TEXTS = {
    "ru": {
        ...
        "my_reports_status_approved": "✅ Жалоба #{id}\n\nСтатус: принята",
        "my_reports_status_rejected": "❌ Жалоба #{id}\n\nСтатус: отклонена\n\nПричина: {reason}",
        "my_reports_status_blocked": "🚫 Жалоба #{id}\n\nСтатус: материал заблокирован",
    ...
    "ua": {
        ...
        "my_reports_status_approved": "✅ Скарга #{id}\n\nСтатус: прийнято",
        "my_reports_status_rejected": "❌ Скарга #{id}\n\nСтатус: відхилено\n\nПричина: {reason}",
        "my_reports_status_blocked": "🚫 Скарга #{id}\n\nСтатус: матеріал заблоковано",
    ...
    "en": {
        ...
        "my_reports_status_approved": "✅ Report #{id}\n\nStatus: approved",
        "my_reports_status_rejected": "❌ Report #{id}\n\nStatus: rejected\n\nReason: {reason}",
        "my_reports_status_blocked": "🚫 Report #{id}\n\nStatus: content has been blocked",
        "blocked_btn": "🚫 Blocked",
...
async def my_report_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if status == "approved":
        ...
        text = TEXTS[lang]["my_reports_status_rejected"].format(id=rid, reason=r.get("reject_reason") or "-")
    elif status == "blocked":
        text = TEXTS[lang]["my_reports_status_blocked"].format(id=rid)
    else:
...
async def cb_view_report(callback: CallbackQuery) -> None:
    ...
    await edit_mod_message(
        callback.bot, callback.message.chat.id, callback.message.message_id,
        report_caption(rid, r), reply_markup=kb_report_detail(rid, r["user_id"])
    )
...
async def process_reject_reason(message: Message, state: FSMContext) -> None:
    ...
    try:
        # Пользователю не отправляем сообщение напрямую — причину отказа
        # он увидит в разделе "Мои жалобы"
        r["status"] = "rejected"
        ...
        await message.answer(f"✅ Жалоба #{rid} отклонена. Причина будет видна пользователю в разделе «Мои жалобы».")
...
async def mod_blocked(callback: CallbackQuery) -> None:
    ...
    try:
        await send_with_photo(
            callback.message,
            TEXTS[lang]["blocked"].format(link=r["link"], type=type_noun),
            chat_id=r["user_id"]
        )
        r["status"] = "blocked"
        ...
        logger.info(f"🚫 По жалобе #{rid} отправлено уведомление о блокировке")
    except Exception as e:
        await callback.message.answer(f"❌ Ошибка: {e}")
