...
@router.message(Command("block"), is_mod_chat)
            ...
            try:
                ...
                if bot_messages:
                    ...
                    for i in range(reports_per_type):
                        ...
                        ok, err_text = await send_message_report(
                            session_client, bot_entity, last_msg.id,
                            selected_reason, text_variant
                        )
                        logger.info(f"[{session_name}] Жалоба на сообщение #{i+1}: {'OK' if ok else 'FAIL: ' + err_text}")
                        if ok:
                            success_count += 1
                            current_reports += 1
                            await update_status()
                            ...
                            error_count += 1
                            current_reports += 1
                            await update_status(f"⚠️ [{session_name}] сообщение #{i+1}: {err_text[:200]}")
                        await asyncio.sleep(random.uniform(2.5, 4.5))
                else:
                    # У этого аккаунта нет истории сообщений с ботом — жалобу на
                    # конкретное сообщение отправить нечем. Уменьшаем общий план,
                    # чтобы прогресс не "зависал" на середине, а корректно показывал 100%.
                    total_reports -= reports_per_type
                    logger.info(f"[{session_name}] Сообщений бота не найдено — пропускаю жалобы на сообщения, план уменьшен до {total_reports}")
                    await update_status(f"ℹ️ {session_name}: нет сообщений от бота, жалуюсь только на профиль")

                for i in range(reports_per_type):
                    if stop_flags.get(task_id, False):
                        break
                    text_variant = random.choice(text_variations)
                    ok, err_text = await send_peer_report(
                        session_client, bot_entity,
                        selected_reason, text_variant
                    )
                    logger.info(f"[{session_name}] Жалоба на профиль #{i+1}: {'OK' if ok else 'FAIL: ' + err_text}")
                    if ok:
                        success_count += 1
                        current_reports += 1
                        await update_status()
                        ...
                        error_count += 1
                        current_reports += 1
                        await update_status(f"⚠️ [{session_name}] профиль #{i+1}: {err_text[:200]}")
                    await asyncio.sleep(random.uniform(2.5, 4.5))
            except Exception as e:
                err_text = f"{type(e).__name__}: {e}"
                logger.error(f"Ошибка сессии {session_name}: {err_text}")
                error_count += 1
                await update_status(f"⚠️ [{session_name}] сбой сессии: {err_text[:200]}")
