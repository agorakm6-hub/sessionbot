...
TEXTS = {
    "ru": {
        ...
        "instruction_text": (
            "1️⃣ Введите ссылку/юзернейм с запрещённым контентом\n"
            "2️⃣ Опишите причину\n"
            "3️⃣ Статус смотрите в «Мои жалобы»\n"
            "4️⃣ Одобрят — объект заблокируют\n"
            "5️⃣ Не по жалобе? Кнопка «Вопрос»"
        ),
    ...
    "ua": {
        ...
        "instruction_text": (
            "1️⃣ Введіть посилання/юзернейм із забороненим контентом\n"
            "2️⃣ Опишіть причину\n"
            "3️⃣ Статус дивіться в «Мої скарги»\n"
            "4️⃣ Схвалять — об'єкт заблокують\n"
            "5️⃣ Не про скаргу? Кнопка «Питання»"
        ),
    ...
    "en": {
        ...
        "instruction_text": (
            "1️⃣ Enter the link/username with prohibited content\n"
            "2️⃣ Describe the reason\n"
            "3️⃣ Check status in \"My reports\"\n"
            "4️⃣ Approved - the object gets blocked\n"
            "5️⃣ Unrelated question? Use \"Question\""
        ),
...
def kb_my_questions_list(user_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    for qid, q in sorted(user_questions, key=lambda x: -x[0]):
        ...
        label_key = "my_questions_item_done" if answered else "my_questions_item_pending"
        rows.append([InlineKeyboardButton(
            text=TEXTS[lang][label_key].format(id=qid),
            callback_data=f"my_question_view_{qid}",
            style="danger" if answered else "primary"
        )])
    if not rows:
...
async def my_question_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # Вопрос должен существовать и принадлежать этому пользователю
...
async def menu_instruction(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.answer(TEXTS[lang]["instruction_text"], show_alert=True)
