...
async def on_startup(app: web.Application) -> None:
    """При запуске сервера - автоматически определяет URL"""
    # Получаем URL из запроса (Render сам подставит)
    # Используем схему и хост из первого запроса
    webhook_url = f"https://{os.getenv('RENDER_EXTERNAL_HOSTNAME', 'localhost')}{WEBHOOK_PATH}"
    
    # Если не на Render - пробуем определить из переменных
