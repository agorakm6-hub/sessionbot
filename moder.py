...
async def my_question_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    async def return_to_list():
        ...
        # Проверяем, не ушёл ли пользователь из этого состояния
        current_state = await state.get_state()
        if current_state is None:
            # Пользователь мог нажать "Назад", ничего не делаем
            return
...
async def my_report_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if status == "approved":
        ...
        # При блокировке отправляем отдельное уведомление пользователю
        try:
            await send_with_photo(
                callback.message,
                TEXTS[lang]["blocked_notification"].format(id=rid),
    ...
    async def return_to_list():
        ...
        if current_state is None:
            # Пользователь мог нажать "Назад", ничего не делаем
            return
        # Возвращаемся к списку репортов
        await render_screen(
            callback.message,
            state,
            TEXTS[lang]["my_reports_title"],
...
async def process_reject_reason(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        try:
            await send_with_photo(
                message,
                TEXTS[lang]["rejected_with_reason"].format(id=rid, reason=reason),
                chat_id=r["user_id"]
            )
        except Exception as e:
            logger.error(f"Ошибка отправки причины отказа пользователю: {e}")
        ...
        await message.answer(f"✅ Репорт #{rid} отклонён. Причина отправлена пользователю.")
    except Exception as e:
...
async def process_question_reply(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer(f"✅ Ответ на вопрос #{qid} отправлен пользователю")
