...
С поддержкой webhook на порту 10000 для стабильного деплоя.
...
# В Bot API для supergroup нужен ID с префиксом -100
# Если ссылка tg://chat?id=4354663980, то ID = -1004354663980
...
async def process_report_confirm(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        if data.get("proof_type") == "photo":
            ...
            logger.info(f"✅ Жалоба с фото отправлена в чат модераторов {MOD_CHAT_ID}")
        else:
            caption += f"📎 Доказательство: {data.get('proof', '-')}"
            await callback.bot.send_message(MOD_CHAT_ID, caption)
            logger.info(f"✅ Жалоба с текстом отправлена в чат модераторов {MOD_CHAT_ID}")
    except Exception as e:
        logger.error(f"❌ Не удалось отправить жалобу в чат модераторов: {e}")
...
async def on_startup(app: web.Application) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Чат модераторов найден: {chat.title} (ID: {MOD_CHAT_ID})")
        except Exception as e:
            logger.warning(f"⚠️ Не удалось получить информацию о чате модераторов: {e}")
