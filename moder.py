...
TEXTS = {
    "ru": {
        ...
        "rejected_with_msg": "❌ Ваш репорт #{id} отклонён.\n\nПричина: {msg}",
      ...
      "ua": {
        ...
        "rejected_with_msg": "❌ Ваш репорт #{id} відхилено.\n\nПричина: {msg}",
        ...
        "blocked": (
            "Вітаємо! Ви надіслали репорт на:\n{link}\n\n"
...
async def my_question_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await menu_my_questions(callback, state)
...
async def my_report_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if status == "approved":
        ...
        asyncio.create_task(_auto_delete_report(message=callback.message, rid=rid, delay=10))
    else:
    ...
    await menu_my_reports(callback, state)

async def _auto_delete_report(message: Message, rid: int, delay: int = 10) -> None:
    await asyncio.sleep(delay)
    if rid in REPORTS:
        del REPORTS[rid]
        logger.info(f"🗑️ Репорт #{rid} удалён после просмотра пользователем")
