...
# ============ .botreport — ОТПРАВКА ЖАЛОБ (как в исходном скрипте) ============
...
async def handle_botreport(message: Message) -> None:
    ...
    status_msg = await message.answer(f"🔄 [BOTREPORT] Начинаем репорт на {bot_username}...")
    ...
    try:
        ...
        await status_msg.edit_text(f"✨ [BOTREPORT] Закончено. Всего: {total_reports} | Успешно: {success_count} | Ошибок: {error_count}")
