...
async def _auto_delete_message(bot: Bot, chat_id: int, message_id: int, delay: int = 10) -> None:
    """Ждёт delay секунд (даёт модератору время прочитать) и удаляет сообщение."""
...
async def edit_mod_message(bot: Bot, chat_id: int, message_id: int, text: str, reply_markup=None) -> None:
    """Редактирует сообщение в чате модераторов: пробует как обычный текст,
    а если это сообщение с фото — редактирует подпись. Никогда не удаляет сообщение."""
    try:
...
TEXTS = {
    "ru": {
        ...
        "report_btn": "📝 Репорт",
        "question_btn": "❓ Вопрос",
        "my_reports_btn": "📄 Мои жалобы",
        "instruction_btn": "ℹ️ Инструкция",
        "instruction_text": (
            "ℹ️ Как пользоваться ботом:\n\n"
            "1️⃣ Введите ссылку или юзернейм, на котором вы наткнулись на запрещённый контент.\n\n"
            "2️⃣ Опишите кратко, что именно нарушает правила (причина жалобы).\n\n"
            "3️⃣ Жалоба уйдёт модераторам — дождитесь решения, статус можно проверить в разделе «Мои жалобы».\n\n"
            "4️⃣ Если жалобу одобрят, объект (бот/канал/сайт) заблокируют, а вы получите уведомление.\n\n"
            "5️⃣ Есть вопрос не по жалобе? Используйте кнопку «Вопрос» — модератор ответит вам лично."
        ),
        "enter_link": "Введите ссылку на нарушающий материал:",
        "enter_reason": "Опишите суть жалобы:",
        "sent": "Ваша жалоба #{id} отправлена на рассмотрение.",
        "back_btn": "⬅️ Вернуться",
        "approved": "✅ Ваша жалоба #{id} принята",
        "rejected": "❌ Ваша жалоба #{id} отклонена",
        "rejected_with_msg": "❌ Ваша жалоба #{id} отклонена.\n\nПричина: {msg}",
        "cooldown": "⏳ Действует кулдаун. Подождите {minutes} мин.",
        "my_reports_title": "📋 Ваши жалобы:",
        "my_reports_empty": "У вас пока нет жалоб.",
        "my_reports_item_pending": "⏳ Жалоба #{id}",
        "my_reports_item_done": "🔔 Жалоба #{id}",
        "my_reports_not_found": "Жалоба не найдена",
        "my_reports_status_pending": "⏳ Жалоба #{id}\n\nСтатус: на рассмотрении",
        "my_reports_status_approved": "✅ Жалоба #{id} принята и будет рассматриваться модераторами",
        "my_reports_status_rejected": "❌ Ваша жалоба #{id} была отклонена",
        "my_reports_status_blocked": "🚫 Жалоба #{id}\n\nСтатус: {type} заблокирован",
    ...
    "ua": {
        ...
        "my_reports_btn": "📄 Мої скарги",
        "instruction_btn": "ℹ️ Інструкція",
        "instruction_text": (
            "ℹ️ Як користуватися ботом:\n\n"
            "1️⃣ Введіть посилання або юзернейм, де ви натрапили на заборонений контент.\n\n"
            "2️⃣ Коротко опишіть, що саме порушує правила (причина скарги).\n\n"
            "3️⃣ Скаргу буде надіслано модераторам — зачекайте рішення, статус можна перевірити в розділі «Мої скарги».\n\n"
            "4️⃣ Якщо скаргу схвалять, об'єкт (бот/канал/сайт) заблокують, а ви отримаєте сповіщення.\n\n"
            "5️⃣ Є питання не про скаргу? Скористайтеся кнопкою «Питання» — модератор відповість вам особисто."
        ),
        "enter_link": "Введіть посилання на матеріал, що порушує правила:",
        "enter_reason": "Опишіть суть скарги:",
        "sent": "Вашу скаргу #{id} надіслано на розгляд.",
        "back_btn": "⬅️ Повернутися",
        "approved": "✅ Вашу скаргу #{id} прийнято",
        "rejected": "❌ Вашу скаргу #{id} відхилено",
        "rejected_with_msg": "❌ Вашу скаргу #{id} відхилено.\n\nПричина: {msg}",
        "cooldown": "⏳ Діє кулдаун. Зачекайте {minutes} хв.",
        "my_reports_title": "📋 Ваші скарги:",
        "my_reports_empty": "У вас поки немає скарг.",
        "my_reports_item_pending": "⏳ Скарга #{id}",
        "my_reports_item_done": "🔔 Скарга #{id}",
        "my_reports_not_found": "Скаргу не знайдено",
        "my_reports_status_pending": "⏳ Скарга #{id}\n\nСтатус: на розгляді",
        "my_reports_status_approved": "✅ Скаргу #{id} прийнято, її розглянуть модератори",
        "my_reports_status_rejected": "❌ Вашу скаргу #{id} відхилено",
        "my_reports_status_blocked": "🚫 Скарга #{id}\n\nСтатус: {type} заблоковано",
    ...
    "en": {
        ...
        "instruction_text": (
            ...
            "1️⃣ Enter the link or username where you found prohibited content.\n\n"
            "2️⃣ Briefly describe what exactly breaks the rules (your reason for reporting).\n\n"
            "3️⃣ Your report is sent to the moderators — wait for a decision, you can check the status in \"My reports\".\n\n"
            "4️⃣ If the report is approved, the object (bot/channel/website) gets blocked and you'll be notified.\n\n"
            "5️⃣ Have a question unrelated to a report? Use the \"Question\" button — a moderator will reply to you personally."
        ),
        ...
        "rejected_with_msg": "❌ Your report #{id} has been rejected.\n\nReason: {msg}",
...
def kb_my_reports_list(user_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    for rid, r in sorted(user_reports, key=lambda x: -x[0]):
        ...
        label_key = "my_reports_item_done" if decided else "my_reports_item_pending"
        rows.append([InlineKeyboardButton(
            text=TEXTS[lang][label_key].format(id=rid),
            callback_data=f"my_report_view_{rid}",
            style="danger" if decided else "primary"
        )])
    if not rows:
...
def kb_moderator_actions(report_id: int, user_id: int) -> InlineKeyboardMarkup:
    # Первичное уведомление о жалобе: только принять/отклонить/бан.
    # Подтверждение блокировки доступно только из вкладки /reports (детальный просмотр).
...
def kb_reports_list() -> InlineKeyboardMarkup:
    # Показывает все актуальные жалобы (кроме отклонённых — те исчезают сами).
    # 🟢 принятые (approved/blocked), 🔴 ещё не принятые (pending).
    rows = []
...
async def render_screen(message: Message, state: FSMContext, text: str, reply_markup=None) -> None:
    """Показывает экран пользователю, редактируя предыдущее сообщение бота вместо его удаления.
    Если предыдущего сообщения нет (или его не удалось отредактировать), отправляет новое."""
    data = await state.get_data()
...
async def menu_report(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if remaining > 0:
        # Показываем всплывающее окно с кулдауном вместо сообщения в чат
...
async def my_report_view(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    # Жалоба должна существовать и принадлежать этому пользователю
...
async def cb_reports_list(callback: CallbackQuery) -> None:
    ...
    await edit_mod_message(
        ...
        "📋 Список активных жалоб:", reply_markup=kb_reports_list()
    )
...
async def mod_approve(callback: CallbackQuery) -> None:
    ...
    try:
        # Пользователю не отправляем сообщение напрямую — статус появится
        # у него в разделе "Мои жалобы"
        r["status"] = "approved"
...
async def process_reject_reason(message: Message, state: FSMContext) -> None:
    ...
    try:
        # Пользователю не отправляем сообщение напрямую — статус "отклонена"
        # он увидит в разделе "Мои жалобы". Причина остаётся только для модераторов.
        r["status"] = "rejected"
        ...
        # Карточка жалобы больше не нужна — удаляем через 10 секунд,
        # чтобы модератор успел прочитать причину отказа.
        ...
        await message.answer(f"✅ Жалоба #{rid} отклонена.")
...
# ============ БЛОКИРОВКА ЗАЯВЛЕННОГО ОБЪЕКТА ============
...
async def mod_blocked(callback: CallbackQuery) -> None:
    ...
    try:
        # Короткое уведомление вместо длинного письма — тип объекта
        # (бот/канал/сайт) определяется автоматически по ссылке.
        await send_with_photo(
            callback.message,
            TEXTS[lang]["my_reports_status_blocked"].format(id=rid, type=type_noun),
            chat_id=r["user_id"]
        )
        r["status"] = "blocked"
        ...
        logger.info(f"🚫 По жалобе #{rid} подтверждена блокировка ({type_noun})")
    except Exception as e:
        await callback.message.answer(f"❌ Ошибка: {e}")
...
async def process_question_reply(message: Message, state: FSMContext) -> None:
    ...
    try:
        await send_with_photo(
            message,
            TEXTS[lang]["question_reply_format"].format(id=qid, answer=answer),
            chat_id=q["user_id"]
        )
        q["answered"] = True
