...
def check_button_cooldown(user_id: int) -> bool:
    """Проверяет, не нажат ли пользователь кнопку слишком часто (5 секунд кулдаун)"""
...
TEXTS = {
    "ru": {
        ...
        "button_cooldown": "⏳ Подождите 5 секунд перед следующим действием",
    },
      "ua": {
        ...
        "button_cooldown": "⏳ Зачекайте 5 секунд перед наступною дією",
    },
    "en": {
        ...
        "button_cooldown": "⏳ Please wait 5 seconds before next action",
    },
}
...
# ============ ГЛОБАЛЬНАЯ БЛОКИРОВКА ВЕЧНОБАНЕНЫХ ============
# Срабатывает раньше любого хендлера: если пользователь в вечном бане и уже
# получил единственное уведомление об этом — бот полностью его игнорирует
# (ни сообщений, ни ошибок), какую бы кнопку/команду он ни отправил.

def _is_silenced(user_id: int) -> bool:
    b = BANNED_USERS.get(user_id)
    return bool(b and b.get("permanent") and b.get("permanent_notified"))

@router.message.outer_middleware()
async def block_permanently_banned_messages(handler, event: Message, data):
    user_id = event.from_user.id if event.from_user else None
    if user_id and event.chat.id != MOD_CHAT_ID and _is_silenced(user_id):
        return
    return await handler(event, data)

@router.callback_query.outer_middleware()
async def block_permanently_banned_callbacks(handler, event: CallbackQuery, data):
    user_id = event.from_user.id if event.from_user else None
    if user_id and event.message and event.message.chat.id != MOD_CHAT_ID and _is_silenced(user_id):
        try:
            await event.answer()  # просто гасим "часики" на кнопке, без текста
        except Exception:
            pass
        return
    return await handler(event, data)
