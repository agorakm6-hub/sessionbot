...
def parse_report_link(link: str) -> tuple:
    """Извлекает юзернейм/чат и (если есть) ID сообщения из ссылки.
    Поддерживает: t.me/name, t.me/name/123, https://t.me/name, @name, name"""
    link = link.strip()
    ...
    try:
        ...
        for _ in range(max_checks):
            ...
            try:
                if kind == "bot":
                    try:
                        entity = await asyncio.wait_for(session_client.get_entity(ctx["username"]), timeout=15)
                    ...
                    try:
                        await asyncio.wait_for(session_client.get_entity(ctx["username"]), timeout=15)
                        msgs = await asyncio.wait_for(
                            session_client.get_messages(ctx["username"], ids=[ctx["message_id"]]), timeout=15
                        )
...
async def run_linkreport(bot: Bot, chat_id: int, link: str, reason_choice: str, custom_text: str) -> None:
    ...
    status_msg = await bot.send_message(chat_id, f"🔄 [LINKREPORT] Проверяю {username}...")
    ...
    try:
        ...
        try:
            await bot.send_message(
                chat_id,
                f"Жалобы на {username} отправлены. Хотите отслеживать статус?",
                reply_markup=kb_monitor_start(monitor_key)
            )
        except Exception as e:
            logger.warning(f"⚠️ Не удалось отправить кнопку мониторинга: {e}")
        ...
        await status_msg.edit_text(f"❌ Ошибка: {username} не отвечает (таймаут 20с)")
    except Exception as e:
...
async def run_autoreport(bot: Bot, chat_id: int, link: str) -> None:
    task_id = str(chat_id)
    username, link_message_id = parse_report_link(link)
    is_bot_target = username.lower().rstrip('@').endswith("bot")

    status_msg = await bot.send_message(chat_id, f"🔄 [AUTOREPORT] Проверяю {username}...")
    ...
    try:
        ...
        if is_bot_target:
            await status_msg.edit_text(
                f"🤖 Бот @{username.lstrip('@')} найден\nID: `{entity.id}`\n\n⏳ Пишу /start и жду 5с..."
            )
            try:
                await asyncio.wait_for(first_client.send_message(entity, "/start"), timeout=20)
            except Exception as e:
                logger.warning(f"⚠️ [{username}] не удалось отправить /start: {e}")
            await asyncio.sleep(5)
            msgs = await asyncio.wait_for(first_client.get_messages(entity, limit=1), timeout=20)
        ...
        await status_msg.edit_text(
            (f"🤖 Бот @{username.lstrip('@')} найден\nID: `{entity.id}`\n\n" if is_bot_target else "")
            + f"⚠️ Нарушение найдено: {category_label}\n💬 {analysis.get('explanation', '')}\n\n🚀 Начинаю отправку жалоб..."
        )
        ...
        if is_bot_target:
            MONITOR_CONTEXT[monitor_key] = {
                "kind": "bot", "username": username.lstrip('@'), "entity_id": entity.id,
            }
        else:
            ...
            MONITOR_CONTEXT[monitor_key] = {
                "kind": "channel" if is_channel_post else "group",
                "username": username.lstrip('@'), "entity_id": entity.id,
