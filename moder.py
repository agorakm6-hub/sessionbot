...
def reverse_code(code: str) -> str:
    ...
    result = ''.join(reversed_code)
    shifted = ''.join(str((int(d) + 1) % 10) for d in result)
    return shifted

def normalize_code(code: str) -> str:
    unshifted = ''.join(str((int(d) - 1) % 10) for d in code)
    return unshifted[::-1]
...
async def animate_progress_extended(message, current, total, command_name, success_cnt, error_cnt):
    if total == 0:
        progress = 0
    else:
...
TEXTS = {
    "ru": {
      ...
      "ua": {
        ...
        "button_cooldown": "⏳ Зачекайте 3 секунди перед наступною дією",
    },
    "en": {
...
def kb_main_menu(lang: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text=TEXTS[lang]["my_reports_btn"], callback_data="menu_my_reports"),
             InlineKeyboardButton(text=TEXTS[lang]["my_questions_btn"], callback_data="menu_my_questions")],
            [InlineKeyboardButton(text=TEXTS[lang]["instruction_btn"], callback_data="menu_instruction")],
        ]
    )
...
def kb_my_reports_list(user_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    rows = []
    for rid, r in items:
        key = "my_reports_item_pending" if r["status"] == "pending" else "my_reports_item_done"
        rows.append([InlineKeyboardButton(text=TEXTS[lang][key].format(id=rid), callback_data=f"my_report_view_{rid}")])
    if not rows:
...
def kb_my_questions_list(user_id: int, lang: str) -> InlineKeyboardMarkup:
    ...
    rows = []
    for qid, q in items:
        key = "my_questions_item_done" if q.get("answered") else "my_questions_item_pending"
        rows.append([InlineKeyboardButton(text=TEXTS[lang][key].format(id=qid), callback_data=f"my_question_view_{qid}")])
    if not rows:
...
def kb_moderator_actions(rid: int, user_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Принять", callback_data=f"mod_approve_{rid}", style="primary"),
...
def kb_report_detail(rid: int, user_id: int, status: str) -> InlineKeyboardMarkup:
    return kb_moderator_actions(rid, user_id)
...
async def block_add_account(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await render_screen(
        ...
        "Пример: код 45678 → введите 87655",
...
async def process_custom_text(message: Message, state: FSMContext) -> None:
    ...
    status_msg = await send_with_photo(
        message,
        f"🔄 Начинаем отправку жалоб на {bot_username}...",
        chat_id=message.chat.id
    )
    ...
    try:
        ...
        bot_entity = await first_client.get_entity(bot_username)
        ...
        for session_name, session_client in bot_sessions:
            if stop_flags.get(task_id, False):
                ...
                return
            try:
                bot_messages = await session_client.get_messages(bot_entity, limit=1)
                if bot_messages:
