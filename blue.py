...
👤 <b>От пользователя:</b>
• Имя: {user.get('first_name', 'Unknown')}
• Юзернейм: @{user.get('username', 'Unknown')}
• ID: {user.get('user_id', 'Unknown')}

🔗 <b>На канал/бот:</b> {report.get('target', 'N/A')}

⚠️ <b>Причина нарушения:</b>
{report.get('reason', 'N/A')}

📸 <b>Доказательство:</b>
{report.get('evidence', 'N/A')}

📌 <b>ID жалобы:</b> {report_id}
🕐 <b>Время:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
"""

        accept_button = InlineKeyboardButton('✅ Принять', callback_data=f'accept_{report_id}')
        reject_button = InlineKeyboardButton('❌ Отклонить', callback_data=f'reject_{report_id}')
        keyboard = [[accept_button, reject_button]]
        reply_markup = InlineKeyboardMarkup(keyboard)

        try:
            await context.bot.send_message(chat_id=MODERATOR_CHAT_ID, text=mod_message, parse_mode='HTML', reply_markup=reply_markup)
        ...
        lang = context.user_data.get('language', 'en')
        keyboard = [[InlineKeyboardButton(get_text(lang, 'edit_target'), callback_data='edit_target'), InlineKeyboardButton(get_text(lang, 'edit_reason'), callback_data='edit_reason')], [InlineKeyboardButton(get_text(lang, 'edit_evidence'), callback_data='edit_evidence')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
  ...
  async def start_app():
    ...
    conversation_handler = ConversationHandler(
        ...
        states={
            ...
            REPORT_CONFIRM: [CallbackQueryHandler(confirm_report, pattern='^confirm_report$|^edit_report$|^back_main$|^edit_target$|^edit_reason$|^edit_evidence$|^back_to_confirm$')],
            REPORT_EDIT: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_edit_input), CallbackQueryHandler(confirm_report, pattern='^edit_target$|^edit_reason$|^edit_evidence$|^back_to_confirm$')],
        },
