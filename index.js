...
// --- ПОДКЛЮЧЕНИЕ К АККАУНТУ (MTProto) с правильной инициализацией ---
const client = new TelegramClient(
    ...
    {
        connectionRetries: 5,
        useWSS: false, // Используем TCP, а не WSS для стабильности
    }
);

// --- ЗАПУСК КЛИЕНТА И БОТА ПОСЛЕДОВАТЕЛЬНО ---
(async () => {
    try {
        ...
        // ТОЛЬКО ПОСЛЕ УСПЕШНОЙ АВТОРИЗАЦИИ запускаем бота
        await bot.launch({
...
// 2. Получение полной инфы о пользователе (с проверкой)
async function getFullUserInfo(username) {
    try {
        if (!client.connected) {
            console.log('Клиент не подключен, переподключаем...');
...
// ХУЙНЯ ДЛЯ RENDER - ДЕРЖИМ ПОРТ ОТКРЫТЫМ
// Render требует, чтобы процесс держал порт открытым
// Даже при Polling создаем простой HTTP сервер для health check
